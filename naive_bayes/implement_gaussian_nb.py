...
class GaussianNB:
    ...
    def fit(self, Xtrain, Ytrain):
        ...
        nd = np.zeros((1, self.D)) + 1/np.sqrt(2*np.pi)
        dk = np.zeros((self.D, self.K)) 
        for i in range(self.K):
            dk[:,i] = np.var(Xtrain[Ytrain==i], axis = 0)
        dk = dk**(-1/2) #dont forget to inverse sqrt root

        #dot product
        self.nk = nd.dot(dk) #1 x K
    ...
    def predict(self, X):
        ...
        for i in range(self.K):
            ...
            xddk = xu.dot(self.vars[:,i]) #N x 1
            
            #the log of the likelihood term
            likelihood = self.nk[0][i] - xddk #N x K
...
import sklearn
benchmark = sklearn.naive_bayes.GaussianNB()
...
#usually 5-10% lower accuracy than sklearn gaussian model benchmark. did i f smt up? revise derivation
